...
class IncomingRequestAdapter(private val listener: IncomingRequestClickListener): ListAdapter<User, IncomingRequestAdapter.IncomingViewHolder>(USER_COMPARATOR) {

    inner class IncomingViewHolder(private val binding: ItemIncomingRequestBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(user: User) {
            binding.apply {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): IncomingViewHolder {
        ...
        binding.apply {
            ...
            requestCancelButton.setOnClickListener {
                listener.rejectRequest(getItem(incomingViewHolder.absoluteAdapterPosition).id)
            }
        }
...
interface IncomingRequestClickListener {
    ...
    fun rejectRequest(friendId: String)
}